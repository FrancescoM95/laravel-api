...
<div class="row g-4 justify-content-end py-2">
    ...
    <div class="col-4">
        ...
        <input type="text"
            ...
            id="title" name="title" placeholder="Inserisci titolo" value="{{ old('title', $project->title) }}">
        @error('title')
        ...
        @foreach ($technologies as $technology)
            <div class="form-check form-check-inline" id="form-check">
                <input class="form-check-input" type="checkbox" name="technologies[]"
                    ...
                    @if (in_array($technology->id, old('technologies', $prev_techs))) checked @endif>
    ...
    <div class="col-12">
        ...
        <input type="file"
            ...
            id="image" name="image" placeholder="Carica immmagine" value="{{ old('image', $project->image) }}">
        @error('image')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
    ...
    <div class="col-12">
        ...
        <textarea class="form-control @error('content') is-invalid @elseif(old('content', '')) is-valid @enderror"
            id="content" name="content" rows="7" placeholder="Inserisci descizione">{{ old('content', $project->content) }}</textarea>
        @error('content')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
